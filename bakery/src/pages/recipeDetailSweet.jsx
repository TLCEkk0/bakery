...
export default function RecipeDetail() {
  const { id } = useParams();
  const recipeSweet = recipesSweet.find((r) => r.id === id);
  ...
  const scaledIngredients = recipeSweet.ingredients.map((ing) => ({
    ...ing,
    ...
    <div className="min-h-screen flex flex-col items-center py-10 px-4">
      <div className="w-full max-w-3xl rounded-xl overflow-hidden">
        ...
        <div className="bg-pink-100 p-6 text-center rounded-t-xl shadow-md">
          <h2 className="text-3xl font-bold text-gray-800">{recipeSweet.name}</h2>
          <p className="text-gray-700 mt-2">หมวด: {recipeSweet.category}</p>
        </div>
